...
# 1. Import ---------------------------------------------------------------
calls <- read.table("/mnt/ibis/lbernatchez/users/lalec31/RDC_Romaine/03_SR_LR/genotype_SVs_SRLR/07_calls/raw/13013A.table",
                    header = TRUE, 
...
colnames(cand_VCF) <- c('CHROM', 'POS', 'ID', 'REF', 'ALT', 'QUAL', 'FILTER', 'INFO', '13066')


# merge
test_merge_13066 <- merge(x = calls, cand_VCF, by = c('CHROM', 'POS'),
                          all = TRUE)
test_merge_13066$SVTYPE <- sub(".*SVTYPE=([A-Z]+);.*", "\\1", test_merge_13066$INFO)
test_merge_13066$SVLEN  <- as.integer(sub(".*SVLEN=(-?[0-9]+).*", "\\1", test_merge_13066$INFO))
test_merge_13066$SUPP_VEC <- sub(".*;SUPP_VEC=([0-1]+);.*", "\\1", test_merge_13066$INFO)
test_merge_13066$SUPP <- as.integer(sub(".*;SUPP=([0-9]+).*", "\\1", test_merge_13066$INFO))
...
calls_DP0 <- subset(test_merge_13066, DP == 0)
...
table(test_merge_13066$SUPP_VEC[test_merge_13066$DP == 0])


## are they neighbors ?
### try on first chr 
calls_ssa01q <- subset(test_merge_13066, CHROM == 'OV354429.1' & DP < 100)
...
DP1_LR_SVs <- subset(test_merge_13066, SUPP_VEC != '01' & DP > 0)

nrow(DP1_LR_SVs)/nrow(subset(test_merge_13066, SUPP_VEC != '01'))

# Test effect of DP filter
## DP > 0
geno_DP1 <- subset(test_merge_13066, DP > 0)
nrow(geno_DP1)

geno_DP2 <- subset(test_merge_13066, DP > 1)
nrow(geno_DP2)

geno_DP4 <- subset(test_merge_13066, DP > 3)
nrow(geno_DP4)

# Check patterns asso with GQ = 0
geno_GQ0 <- subset(test_merge_13066, GQ == 0)
table(geno_GQ0$SUPP_VEC)
table(geno_GQ0$GT) # most are missing GT ./.

## do GQ0 SVs include ALL DP0 SVs ? : YES
geno_DP0 <- subset(test_merge_13066, DP == 0)
...
## GQ > 0
geno_GQ1 <- subset(test_merge_13066, GQ > 0)
nrow(geno_GQ1)
table(geno_GQ1$SUPP_VEC)
table(geno_GQ1$GT)

geno_GQ2 <- subset(test_merge_13066, GQ > 1)
nrow(geno_GQ2)

geno_GQ4 <- subset(test_merge_13066, GQ > 3)
nrow(geno_GQ4)
...
weird_GTs <- subset(test_merge_13066, ! GT %in% allowed_GTs)

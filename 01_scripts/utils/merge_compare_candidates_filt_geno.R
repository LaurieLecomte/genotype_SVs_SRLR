...
# Compute info on each filtered call
...
# 2. Explore filtered genotyped SVs --------------------------------------------
# How many filtered calls indels, not SVs ?
table(geno$VAR_TYPE)
...
cand <- as.data.frame(cand)
geno <- as.data.frame(geno)
cand_geno <- merge(x = cand[, cand_cols], y = geno[, geno_cols], 
